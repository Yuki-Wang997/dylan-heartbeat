...
console.log("DEBUG ENV FILE:", process.env.ALLOW_PUBLIC_API);
console.log("DEBUG ALL MATCH:", Object.entries(process.env).filter(([k]) => k.includes("ALLOW")));
...
app.post("/v1/wake", async (req, reply) => {
  ...
  setImmediate(async () => {
    ...
    try {
      ...
      console.error("wake触发失败:", err.message);
    }
  });
});
